...
log = structlog.get_logger("tamara.db")
...
class MariaDBClient:
    ...
    ALLOWED_READ_COMMANDS: ClassVar[list[str]] = ["SELECT", "SHOW", "DESCRIBE", "DESC", "EXPLAIN"]
    ...
    def connect(self) -> bool:
        ...
        if not MARIADB_AVAILABLE:
            log.error("mariadb package not installed")
            return False
        ...
        try:
            ...
            log.info("Connected", database=self._database, host=self._host)
            return True
    ...
    def _validate_query(self, query: str) -> None:
        ...
        if first_word not in self.ALLOWED_READ_COMMANDS and not self._allow_write:
            raise SecurityError(
    ...
    def list_tables(self) -> list[str]:
        ...
        return [next(iter(row.values())) for row in results]
...
_db_client: MariaDBClient | None = None


def get_db_client() -> MariaDBClient | None:
    """
